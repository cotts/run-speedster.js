(function (win) {
    ...
    let insertShadow = (el) => {
        if (size === el.config.sizeOfPath - el.config.howBig) return;
        runnerShadow.push({
    ...
    let createBlur = (shadow) => {
        if (blurSize === shadow.length) return;
    ...
    function init(path, text, configs) {
        createRunner(path, text, configs)
            .then(showRunner)
            .then(insertShadow)
            .then(createBlur)
            .then(run);
    ...
    win.RunSpeedster = win.RunSpeedster || {};
    win.RunSpeedster.init = init;
